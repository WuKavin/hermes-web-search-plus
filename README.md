...
This fork adds AnySearch as a production Provider SDK integration for search and extraction, promotes it for multilingual and high-value vertical queries, and keeps Brave in the automatic Routing v2 pool.
...
Web Search Plus supports 14 search and 10 extraction providers in this fork — you do **not** need them all. One search-capable key or configured local endpoint enables `web_search_plus`; one extraction-capable key or endpoint enables `web_extract_plus`; more providers just make controlled routing more flexible. The setup helper stores keys in the active Hermes environment file — never commit them to the repository.
